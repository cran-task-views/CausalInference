...
-   *Construction of experimental designs* is implemented in
    ...
    with time-to-event endpoints), and `r pkg("gsMAMS")` (multi-arm multi-stage designs). Many other tools and packages exist
...
-   *Mediation analysis* can be performed with `r pkg("cit")`
    ...
    `r pkg("bama")` performs Bayesian mediation analysis with high-dimensional mediators, and `r pkg("HIMA")` does the same in a non-Bayesian context. 
